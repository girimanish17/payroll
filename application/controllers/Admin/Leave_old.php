...
class Leave extends CI_Controller
{
	...
	public function index(){
		...
		$data['emp_leaves'] =$emp_leaves= $this->common_model->GetAllData('emp_leaves',array(),'created_date','desc');
		//$data['emp_leaves'] = $this->common_model->all_leaves($where);
		$data['approved_leaves'] =  $this->common_model->count_row('emp_leaves',array('status'=>'Approved'));
		$data['rejected_leaves'] =  $this->common_model->count_row('emp_leaves',array('status'=>'Rejected'));
		$data['pending_leaves']  =  $this->common_model->count_row('emp_leaves',array('status'=>'Pending'));
	     //Year wise
	...
	public function compoff_leave_request(){
		$user_id = $this->session->userdata('user_id');
		$type = $this->session->userdata('user_type'); //
		$data['emp_leaves'] =$emp_leaves= $this->common_model->GetAllData('compoff_emp_leaves',array('leave_status!='=>1),'created_date','desc');
		$data['approved_leaves'] =  $this->common_model->count_row('emp_leaves',array('status'=>'Approved'));
		$data['rejected_leaves'] =  $this->common_model->count_row('emp_leaves',array('status'=>'Rejected'));
		$data['pending_leaves']  =  $this->common_model->count_row('emp_leaves',array('status'=>'Pending'));
		//$where = '';
	...
	public function leave_type(){
		$user_id = $this->session->userdata('user_id');
		$type = $this->session->userdata('user_type');
		$data['leave_type'] = $this->common_model->GetAllData('leave_type',array('status' =>1),'created_date','desc');
